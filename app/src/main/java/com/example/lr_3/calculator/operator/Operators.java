package com.example.lr_3.calculator.operator;
...
public abstract class Operators {
    ...
    static {
        ...
        BUILT_IN_OPERATORS[INDEX_DIVISION] = new Operator("/", 2, true, Operator.PRECEDENCE_DIVISION) {
            @Override
            public BigDecimal apply(final BigDecimal... args) {
                if (args[1].equals(new BigDecimal(0))) {
                    throw new ArithmeticException("Division by zero!");
                }
                return args[0].divide(args[1], 200, RoundingMode.CEILING);
            }
        };
        ...
        BUILT_IN_OPERATORS[INDEX_FACTORIAL] = new Operator("!", 1, true, Operator.PRECEDENCE_POWER) {
            @Override
            public BigDecimal apply(BigDecimal... args) {
                final int arg = Integer.parseInt(args[0].toString());
                if (!BigDecimal.valueOf(arg).equals(args[0])) {
    ...
    public static Operator getBuiltinOperator(final char symbol, final int numArguments) {
        switch (symbol) {
            ...
            case '!':
                return BUILT_IN_OPERATORS[INDEX_FACTORIAL];
            default:
                return null;
        }
    }
